...
console.log('\ntrace-correct · propiedad del lock al liberar');
{
  ...
  writeFileSync(preload, [
    ...
    '  if (process.env.SDD_TEST_LOCK_PATH && fs.existsSync(process.env.SDD_TEST_LOCK_PATH)) {',
    '    lockFstatCalls += 1;',
    "    const shouldReplace = process.platform === 'win32' ? lockFstatCalls === 2 : lockFstatCalls === 1;",
    '    if (!replaced && shouldReplace) {',
    '      replaced = true;',
    "      if (process.platform === 'win32') {",
    '        fs.ftruncateSync(fd, 0);',
    "        fs.writeSync(fd, 'foreign-lock\\n', 0, 'utf8');",
    '        fs.fsyncSync(fd);',
    '      } else {',
